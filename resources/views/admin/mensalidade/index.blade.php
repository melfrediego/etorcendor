        ...
        <div class="m-portlet__head-tools">
            <ul class="m-portlet__nav">
                <li class="m-portlet__nav-item">
                    <a href="/mensalidade" class="m-portlet__nav-link btn btn-brand m-btn m-btn--icon m-btn--icon-only btn-sm btn-metal">
    ...
    <div class="m-portlet__body">
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <form action="{{route('ticket.search')}}" method="POST" class="form-group form-inline">
                    @csrf
                    ...
                    <div class="input-group-append">
                        ...
                        &nbsp;
                        <a href="/ticket" class="btn btn-default btn-sm">
                ...
                <tbody>
                    @forelse ($itens as $key => $item)
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>
                                {{-- <a href="{{ route('socio.show',$item->pedido->socio->id) }}">{{ $item->pedido->socio->nome }}</a> --}}
                                {{ $item->pedido->socio->nome }}
                            </td>
                            <td>{{ $item->numero_mensalidade }} / 12</td>
                            ...
                            <td width="150" class="text-center">
                                ...
                                @else
                                    <span class="m-badge m-badge--danger m-badge--wide"><i class="fa fa-warning"></i> Pendente</span>
                                @endif
                            </td>
                            <td>R$ {{ $item->valor_cobrado }}</td>
                            {{-- <td width="140">{{ date('d/m/Y', strtotime($item->created_at))}}</td> --}}
                            {{-- <td width="140">{{ date('d/m/Y', strtotime($item->updated_at))}}</td> --}}
                            <td width="60">
                                <a href="{{ route('mensalidade.show',$item->id) }}" class="btn btn-primary m-btn m-btn--icon btn-sm m-btn--icon-only" data-skin="dark" data-toggle="m-tooltip" data-placement="top" title="" data-original-title="Ver dados"><i class="fa fa-search"></i></a>
                                {{-- <a href="{{ route('mensalidade.edit',$item->id) }}" class="btn btn-sm btn-info" data-skin="dark" data-toggle="m-tooltip" data-placement="top" title="" data-original-title="Editar dados"><i class="fa fa-edit"></i></a> --}}
                    ...
                    @empty
                        <h2 class="text-center">Nenhuma mensalidade cadastrada</h2>
                    @endforelse
